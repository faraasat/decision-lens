...
export default function Dashboard() {
  ...
  if (!data) {
    ...
    isLive &&
    currentTimeIndex >= 0 &&
    Array.isArray(data?.timeline_snapshots) &&
    data.timeline_snapshots.length > 0 &&
    currentTimeIndex < data.timeline_snapshots.length &&
    data.timeline_snapshots[currentTimeIndex]
  ) {
    ...
    currentData = {
      ...
      current_state: {
        ...
        gold_diff: snap.gold_diff ?? 0,
        xp_diff: snap.xp_diff ?? 0,
        timestamp: snap.timestamp ?? 0,
      },
      player_stats: snap.player_stats || [],
      shap_explanations: snap.shap_explanations || {},
      macro_insights: Array.isArray(data.macro_insights)
        ? data.macro_insights.filter(
            (m: any) => m.timestamp <= (snap.timestamp ?? 0),
          )
        : [],
      micro_insights: Array.isArray(data.micro_insights)
        ? data.micro_insights.filter(
            (m: any) => m.timestamp <= (snap.timestamp ?? 0),
          )
        : [],
    ...
    const lastSnap =
      Array.isArray(data?.timeline_snapshots) &&
      data.timeline_snapshots.length > 0
        ? data.timeline_snapshots[data.timeline_snapshots.length - 1]
        : undefined;
    currentData = {
      ...data,
      ...
      shap_explanations:
        lastSnap?.shap_explanations || data?.shap_explanations || {},
      macro_insights: Array.isArray(data?.macro_insights)
        ? data.macro_insights
        : [],
      micro_insights: Array.isArray(data?.micro_insights)
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#050a14] flex flex-col items-center justify-center">
        ...
        <p className="text-primary font-mono tracking-widest animate-pulse uppercase">
          Initializing Neural Link for {activeGame.toUpperCase()}...
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-[#050a14] flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-slate-400 mb-8 max-w-md">
          Failed to establish connection with the decision engine. Ensure the
  ...
  if (!loading && (!liveMatches || liveMatches.length === 0) && !data) {
    return (
      <div className="min-h-screen bg-[#050a14] flex flex-col items-center justify-center p-6 text-center text-white">
        ...
        <h2 className="text-3xl font-bold mb-4 tracking-tighter uppercase font-mono">
          No Live Matches Found
        </h2>
        <p className="text-slate-400 mb-8 max-w-md">
          There are currently no active {activeGame.toUpperCase()} series on the
          GRID Network. Please check back later or try a different title.
        </p>
  ...
  const snapshotsToUse =
    isLive && currentTimeIndex >= 0 && Array.isArray(data?.timeline_snapshots)
  ...
  const chartData =
    Array.isArray(snapshotsToUse) && snapshotsToUse.length > 0 && data
      ? snapshotsToUse.map((s: any) => {
          const timeInMin = Math.floor((s.timestamp ?? 0) / 60000);
          // Find events at this minute
          const minuteEvents = Array.isArray(data?.objectives)
            ? data.objectives.filter(
                (o: any) =>
                  Math.floor((o.timestamp ?? 0) / 60000) === timeInMin,
              )
            : [];
          const kills = Array.isArray(data?.micro_insights)
            ? data.micro_insights.filter(
                (m: any) =>
                  Math.floor((m.timestamp ?? 0) / 60000) === timeInMin,
              )
            : [];
          return {
            time: `${timeInMin}:00`,
            ...
            prob: s.win_prob ?? 0.5,
            goldDiff: s.gold_diff ?? 0,
            xpDiff: s.xp_diff ?? 0,
            event: minuteEvents.length > 0 ? minuteEvents[0].type : null,
            isKill: kills.length > 0,
          };
        })
      : [{ time: "0:00", prob: 0.5, goldDiff: 0, xpDiff: 0 }];

  const currentExplanations =
    isSimulated && simulationResult
      ? simulationResult.shap_explanations
      : currentData?.shap_explanations;

  const shapData =
    currentExplanations && typeof currentExplanations === "object"
      ? Object.entries(currentExplanations)
      ...
      <header className="flex justify-between items-center mb-8 border-b border-blue-900/50 pb-4 relative z-10">
        ...
        <div className="flex gap-6">
          <div className="flex items-center gap-2">
            ...
            >
              <Activity
                className={cn("w-4 h-4", isLive ? "animate-spin" : "")}
              />
            ...
            <div className="text-right">
              ...
              >
                {liveMatches.length > 0 ? (
                  ...
                  <option
                    disabled
                    className="bg-slate-900 text-primary italic text-[10px]"
                  >
                    Awaiting Series Link...
                  </option>
                )}
              </select>
            </div>
            ...
            >
              {isLive ? (
                <>
                  LIVE{" "}
                  {(
                    (currentData?.current_state?.win_prob || 0.5) * 100
                  ).toFixed(1)}
                  %
                </>
              ) : isSimulated ? (
        ...
        <div className="col-span-12 lg:col-span-8 space-y-6">
          <div className="grid grid-cols-4 gap-4">
            {[
              {
                label:
                  currentData?.game === "valorant" ? "Econ Diff" : "Gold Diff",
              ...
              {
                label:
                  currentData?.game === "valorant" ? "Loadout Diff" : "XP Diff",
              ...
              <div
                key={i}
                className="glass-panel p-4 rounded-xl border-l-2 border-primary/50 relative overflow-hidden"
              >
                {isLive && (
                  <div className="absolute top-0 right-0 w-1 h-1 bg-red-500 rounded-full m-1 animate-ping" />
                )}
          ...
          <div className="glass-panel p-6 rounded-2xl relative overflow-hidden">
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex gap-2 items-center">
                ...
                <span className="px-3 py-1 bg-primary/10 text-primary border border-primary/20 rounded text-[10px] font-bold">
                  XGBOOST ENGINE
                </span>
                <span className="px-3 py-1 bg-slate-800 text-slate-400 rounded text-[10px] font-bold uppercase tracking-tighter">
                  Live Inference
                </span>
              </div>
            </div>
            ...
            <div className="p-6">
              {activeTab === "macro" && (
                <div className="space-y-4">
                  {(isLive
                    ? currentData?.macro_insights?.filter(
                        (m: any) =>
                          m.timestamp <= currentData.current_state.timestamp,
                      )
                    : currentData?.macro_insights
                  )?.map((m: any, i: number) => (
                    <div
                      key={i}
                      className="flex items-start gap-4 p-4 rounded-xl bg-slate-900/50 border border-slate-800 hover:border-primary/30 transition-colors group"
                    >
                      ...
                      <div className="flex-1">
                        <div className="flex justify-between">
                          <h4 className="font-bold text-slate-100 uppercase text-sm tracking-tight">
                            {m.type}
                          </h4>
                          <span className="font-mono text-xs text-primary bg-primary/10 px-2 py-0.5 rounded">
                            {Math.floor(m.timestamp / 60000)}:
                            {Math.floor((m.timestamp % 60000) / 1000)
                              .toString()
                              .padStart(2, "0")}
                          </span>
                        </div>
                  ...
                  {(!currentData?.macro_insights ||
                    currentData.macro_insights.length === 0) && (
                    <p className="text-center text-slate-500 py-8 italic">
                      No macro inflections detected in this snapshot.
                    </p>
                  )}
                </div>
              )}
              ...
              {activeTab === "micro" && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {(isLive
                    ? currentData?.micro_insights?.filter(
                        (m: any) =>
                          m.timestamp <= currentData.current_state.timestamp,
                      )
                    : currentData?.micro_insights
                  )?.map((mistake: any, i: number) => (
                    <div
                      key={i}
                      className="p-4 rounded-xl bg-slate-900/50 border border-slate-800 flex justify-between items-center group hover:border-red-500/30 transition-colors"
                    >
                      <div className="flex gap-3 items-center">
                        <div className="w-8 h-8 rounded bg-red-500/10 flex items-center justify-center font-bold text-red-500 text-xs">
                          ERR
                        </div>
                        <div>
                          <p className="text-xs font-bold text-slate-500 uppercase">
                            Player {mistake.player_id}
                          </p>
                          <p className="text-sm font-medium">{mistake.type}</p>
                          <p className="text-[10px] text-slate-600 font-mono mt-1">
                            TS: {Math.floor(mistake.timestamp / 60000)}m
                          </p>
                        </div>
                      </div>
                      <span
                  ...
                  {(!currentData?.micro_insights ||
                    currentData.micro_insights.length === 0) && (
                    <p className="text-center text-slate-500 py-8 italic col-span-2">
                      No individual mistakes identified.
                    </p>
                  )}
                </div>
              )}
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="glass-panel p-4 rounded-xl border-l-2 border-blue-400">
                      ...
                      <p className="text-[10px] text-green-400 font-bold uppercase mt-1">
                        {currentData?.game === "valorant"
                          ? currentData?.current_state?.gold_diff > 1000
                            ? "Full Buy"
                            : "Partial Buy"
                          : `${(
                              (currentData?.player_stats
                    ...
                    <div className="glass-panel p-4 rounded-xl border-l-2 border-primary">
                      ...
                      <p className="text-2xl font-black italic">
                        {currentData?.game === "valorant"
                          ? `${(((currentData?.current_state?.gold_diff || 0) / Math.max(Math.abs(currentData?.current_state?.gold_diff || 1), 1000)) * 50 + 50).toFixed(0)}%`
                          : `${Math.min(95, Math.max(30, 50 + (currentData?.current_state?.gold_diff || 0) / 100)).toFixed(0)}%`}
                      </p>
                      <p className="text-[10px] text-primary font-bold uppercase mt-1">
                        {currentData?.game === "valorant"
                          ? (currentData?.current_state?.gold_diff || 0) > 0
                            ? "Optimal Spend"
                            : "Conservative"
                          : Math.abs(
                                currentData?.current_state?.gold_diff || 0,
                  ...
                  <div className="p-6 rounded-2xl bg-slate-900/40 border border-slate-800">
                    <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-6">
                      {currentData?.game === "valorant"
                        ? "Economy Advantage Timeline"
                        : "Warding Activity Timeline"}
                    </h3>
                ...
                <div className="space-y-8">
                        ...
                        {currentData?.player_stats?.map((p: any) => (
                          ...
                          >
                            ...
                            <td className="py-4 text-xs uppercase font-bold">
                              <span
                                className={
                                  p.team_id === 100 || p.team_id === "team-blue"
                                    ? "text-blue-500"
                                    : "text-red-500"
                                }
                              >
                            ...
                            <td className="py-4 text-right font-mono text-sm">
                              {currentData?.game === "valorant"
                                ? `${p.headshot_percent?.toFixed(1)}%`
                                : p.cs_per_min?.toFixed(1) || "0.0"}
                            </td>
                  ...
                  <div className="col-span-12 xl:col-span-4 space-y-4">
                    <div className="bg-slate-900/40 p-6 rounded-2xl border border-slate-800 h-full">
                      <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-6">
                        Player {selectedPlayer} Performance
                      </h3>
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                          <RadarChart
                            ...
                            data={[
                              ...
                              {
                                subject: "KP%",
                                A: currentData?.player_stats?.find(
                                  (p: any) => p.player_id === selectedPlayer,
                                )?.kill_participation,
                                fullMark: 100,
                              },
                              {
                    ...
                    <div className="bg-slate-900/40 p-6 rounded-2xl border border-slate-800">
                      ...
                      <div className="space-y-4">
                        {(() => {
                          ...
                          const blueDragons = objectives.filter(
                            (o: any) =>
                              (o.type === "DRAGON" ||
                                o.type === "ELITE_MONSTER_KILL") &&
                              (o.team_id === 100 || o.team_id === "team-blue"),
                          ).length;
                          const redDragons = objectives.filter(
                            (o: any) =>
                              (o.type === "DRAGON" ||
                                o.type === "ELITE_MONSTER_KILL") &&
                              (o.team_id === 200 || o.team_id === "team-red"),
                          ).length;
                          const totalDragons = blueDragons + redDragons || 1;

                          const blueTowers = objectives.filter(
                            (o: any) =>
                              (o.type === "TOWER" ||
                                o.type === "BUILDING_KILL") &&
                              (o.team_id === 100 || o.team_id === "team-blue"),
                          ).length;
                          const redTowers = objectives.filter(
                            (o: any) =>
                              (o.type === "TOWER" ||
                                o.type === "BUILDING_KILL") &&
                              (o.team_id === 200 || o.team_id === "team-red"),
                          ).length;
                          const totalTowers = blueTowers + redTowers || 1;
                            ...
                            },
                            {
                              label:
                                currentData?.game === "valorant"
                                  ? "Blue Site Control"
                                  : "Blue Tower Control",
                              val: Math.round((blueTowers / totalTowers) * 100),
                              color: "bg-blue-500",
                            },
                            {
                              label:
                                currentData?.game === "valorant"
                                  ? "Red Site Control"
                                  : "Red Tower Control",
                              val: Math.round((redTowers / totalTowers) * 100),
                              color: "bg-red-500",
                            },
                          ].map((obj, i) => (
                  ...
                  <div className="p-6 rounded-2xl bg-blue-500/5 border border-blue-500/20">
                    ...
                    <div className="grid grid-cols-4 gap-4">
                      {(() => {
                        const objectives = currentData?.objectives || [];
                        const winProb =
                          currentData?.current_state?.win_prob || 0.5;
                        const goldDiff =
                          currentData?.current_state?.gold_diff || 0;

                        // Calculate objective setup score based on objectives secured
                        const objectiveScore = Math.min(
                          95,
                          Math.max(30, 50 + objectives.length * 5),
                        );
                        ...
                        // Map pressure based on gold difference
                        const mapPressure = Math.min(
                          95,
                          Math.max(20, 50 + goldDiff / 50),
                        );
                        const mapStatus =
                          mapPressure > 70
                            ? "Strong"
                        ...
                        // Teamfight prowess based on win probability
                        const teamfightScore = Math.round(winProb * 100);
                        const teamfightStatus =
                          teamfightScore > 65
                            ? "Elite"
                            : teamfightScore > 50
                              ? "Good"
                        ...
                        // Vision control based on player vision scores
                        const avgVision =
                          currentData?.player_stats?.reduce(
                            (sum: number, p: any) =>
                              sum + (p.vision_score || 0),
                            0,
                          ) / (currentData?.player_stats?.length || 1) || 0;
                        const visionScore = Math.min(
                          95,
                          Math.max(20, avgVision * 2),
                        );
                        const visionStatus =
                          visionScore > 60
                            ? "Excellent"
                        ...
                        return [
                          {
                            label:
                              currentData?.game === "valorant"
                                ? "Site Setup"
                                : "Objective Setup",
                            val: `${objectiveScore.toFixed(0)}%`,
                            status: objectiveStatus,
                          },
                          {
                            label: "Map Pressure",
                            val: `${mapPressure.toFixed(0)}%`,
                            status: mapStatus,
                          },
                          {
                            label:
                          ...
                          <div key={i} className="text-center">
                            ...
                            <span className="text-[8px] px-2 py-0.5 rounded bg-slate-800 text-slate-400 uppercase font-bold">
                              {stat.status}
                            </span>
                          </div>
                        ));
              ...
              {activeTab === "draft" && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {data?.metadata?.teams?.map((team: any, i: number) => (
                    <div
                      key={i}
                      className="glass-panel p-6 rounded-2xl border-t-4 border-t-primary"
                    >
                      <div className="flex justify-between items-center mb-6">
                        ...
                        <span
                          className={`text-[10px] font-bold px-3 py-1 rounded-full uppercase ${team.side === "blue" ? "bg-blue-500/20 text-blue-500" : "bg-red-500/20 text-red-500"}`}
                        >
                          {team.side} Side
                        </span>
                      </div>
                      ...
                      <div className="space-y-3">
                        {team.draft?.map((champ: string, idx: number) => (
                          <div
                            key={idx}
                            className="flex items-center gap-4 p-3 bg-slate-900/50 rounded-xl border border-slate-800"
                          >
                      ...
                      <div className="mt-8 pt-6 border-t border-slate-800">
                        <div className="flex justify-between mb-2">
                          ...
                          <span className="text-primary font-black italic">
                            {data?.draft_analysis?.[team.id]?.synergy_score}%
                          </span>
                        </div>
                        <div className="w-full h-2 bg-slate-800 rounded-full overflow-hidden">
                          <div
                            className="h-full bg-primary"
                            style={{
                              width: `${data?.draft_analysis?.[team.id]?.synergy_score}%`,
                            }}
                          />
                        </div>
                        <p className="text-[10px] text-slate-400 mt-4 italic">
                          "This draft favors{" "}
                          {data?.draft_analysis?.[team.id]?.power_spike}{" "}
                          engagements with strong scaling."
                        </p>
                      </div>
                    </div>
                  ))}
                ...
                <div className="space-y-6">
                  <div className="p-8 rounded-3xl border-2 border-dashed border-primary/20 bg-primary/5 text-center relative overflow-hidden group">
                    ...
                    <p className="text-sm text-slate-400 max-w-md mx-auto mt-2 font-medium">
                      Quantify tactical decisions by injecting counterfactual
                      game states into the XGBoost model.
                    </p>
                      ...
                      <div className="flex justify-center gap-4 mt-8">
                        ...
                        >
                          {isSimulating
                            ...
                            : "RUN LIVE SIMULATION"}
                          <RefreshCcw
                            className={`w-6 h-6 ${isSimulating ? "animate-spin" : ""}`}
                          />
                        </button>
                      </div>
                    )}
                    ...
                    >
                      <div className="absolute top-4 right-4">
                        <button
                          ...
                          className="text-[10px] font-black text-slate-500 uppercase hover:text-red-500 transition-colors"
                        >
                          TERMINATE SESSION [X]
                        </button>
                      </div>
                      ...
                      <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                        <div className="flex items-center gap-6">
                          ...
                          <div>
                            <p className="text-xs font-bold text-primary uppercase tracking-[0.3em] mb-1">
                              Projected Outcome
                            </p>
                            <p className="text-5xl font-black italic text-white tracking-tighter">
                              {(
                                (simulationResult?.win_probability || 0) * 100
                              ).toFixed(1)}
                              %
                            </p>
                          </div>
                        ...
                        <div className="flex gap-10 items-center bg-black/40 p-6 rounded-2xl border border-slate-800">
                          <div className="text-center">
                            ...
                            <p
                              className={`text-3xl font-black italic ${simulationResult?.win_probability - (data?.timeline_snapshots?.[data.timeline_snapshots.length - 1]?.win_prob || 0.5) >= 0 ? "text-green-400" : "text-red-400"}`}
                            >
                              {simulationResult?.win_probability -
                                (data?.timeline_snapshots?.[
                                  data.timeline_snapshots.length - 1
                                ]?.win_prob || 0.5) >=
                              0
                                ? "+"
                                : ""}
          ...
          <div className="glass-panel p-6 rounded-2xl h-full border-r border-primary/30 flex flex-col">
            ...
            <div className="space-y-6 flex-1 overflow-auto pr-2 custom-scrollbar">
              <div className="bg-slate-900/80 p-5 rounded-xl border border-slate-800 relative shadow-[0_0_15px_rgba(0,0,0,0.5)]">
                <div className="absolute -top-2 -left-2 bg-primary text-white text-[10px] font-black text-6xl px-2 py-4 italic shadow-[0_0_10px_rgba(0,163,255,0.5)] w-full ml-2 rounded-md mt-2">
                  NEURAL SUMMARY
                </div>
              ...
              <div className="bg-slate-900/40 p-5 rounded-xl border border-slate-800">
                ...
                <p className="text-[8px] text-slate-600 font-mono mt-2 italic text-center uppercase tracking-tighter">
